...
import { isProfileCompletionRequired } from "./_core/profileCompletion";
...
async function confirmTossPayment(
  ...
  if (!response.ok) {
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: err.message ?? "토스페이먼츠 결제 승인 실패",
    });
  }
...
async function cancelTossPayment(paymentKey: string, cancelReason: string) {
  ...
  if (!response.ok) {
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: err.message ?? "토스페이먼츠 결제 취소 실패",
    });
  }
  return response.json();
}

function toTrpcCodeFromStatus(
  status: number
):
  | "BAD_REQUEST"
  | "UNAUTHORIZED"
  ...
  throw new TRPCError({
    ...
    message:
      error instanceof Error
        ? error.message
        : "Supabase Edge Function 호출에 실패했습니다.",
  });
}
...
export const appRouter = router({
  ...
  auth: router({
    me: publicProcedure.query(opts =>
      opts.ctx.user
        ? {
            ...opts.ctx.user,
            requiresProfileCompletion: isProfileCompletionRequired(opts.ctx.user),
          }
        : null
    ),
    ...
    emailSignup: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        throw new TRPCError({
          ...
          message:
            "이메일 가입은 /api/auth/email/signup 엔드포인트를 사용하세요.",
        });
      }),

    // ─── 아이디/이메일 로그인 ─────────────────────────────────────────────────
    emailLogin: publicProcedure
      .input(z.object({ identifier: z.string().min(1), password: z.string() }))
      .mutation(async _opts => {
        // Supabase Auth 표준 로그인 플로우
        // 실제 로그인 처리는 POST /api/auth/email/signin 엔드포인트에서 수행
        throw new TRPCError({
          code: "METHOD_NOT_SUPPORTED",
          message: "로그인은 /api/auth/email/signin 엔드포인트를 사용하세요.",
        });
      }),
    ...
    findLoginId: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const result = await db
          ...
          .from(profilesTable)
          .where(eq(profilesTable.name, input.name.trim()));
        const matched = result.find(
          row => normalizePhone(row.phone ?? "") === normalizePhone(input.phone)
        );
        if (!matched)
          throw new TRPCError({
            code: "NOT_FOUND",
        ...
        if (matched.username) {
          return {
            maskedLoginId: maskUsername(matched.username),
            loginType: "username" as const,
          };
        }
        if (matched.email) {
          return {
            maskedLoginId: maskEmail(matched.email),
    ...
    requestPasswordReset: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const redirectTo = input.origin
          ? `${input.origin}/find-password?mode=reset`
          : undefined;
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!userId && input.token) {
          ...
          const { data: userData, error: userError } =
            await supabaseAdmin.auth.getUser(input.token);
        ...
        if (!userId) {
          throw new TRPCError({
            code: "UNAUTHORIZED",
            message: "비밀번호를 재설정할 수 있는 세션이 없습니다.",
          });
        }
  ...
  certifiedInstructor: router({
    list: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        return getCertifiedInstructorsSupabase({
          publishedOnly: true,
          page: input?.page ?? 1,
          limit: input?.limit ?? 100,
        });
  ...
  verification: router({
    get: protectedProcedure.query(async ({ ctx }) =>
      getLatestVerification(ctx.user.id)
    ),
    submit: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (db) {
          ...
          await db
            .update(profiles)
            .set({ proVerificationStatus: "pending" })
  ...
  admin: router({
    ...
    updateOrderStatus: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const allowedTransitions: Record<
          typeof prevOrder.status,
          Array<typeof prevOrder.status>
        > = {
        ...
        if (!updated) {
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "주문 상태가 이미 변경되었습니다. 새로고침 후 다시 시도해주세요.",
          });
        }
        ...
        if (
          ...
          const productName =
            items.length > 0 ? items[0].productName : "레아쥬 제품";
    ...
    searchOrders: protectedProcedure
      .input(
        z.object({
          ...
          search: z.string().optional(),
          searchType: z
            .enum(["orderId", "name", "email", "productName"])
            .optional(),
    ...
    createGalleryPost: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        return createAdminGalleryPost({
          ...input,
          authorId: ctx.user.id as any,
        });
      }),
    ...
    createMagazinePost: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        return createAdminMagazinePost({
          ...input,
          authorId: ctx.user.id as any,
        });
      }),
    ...
    createReview: protectedProcedure
      .input(
        z.object({
          category: z
            .enum(["before_after", "device", "education", "event", "etc"])
            .default("etc"),
    ...
    // ─── Popups ────────────────────────────────────────────────────────────────
    popups: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      return getPopups();
    }),
    ...
    ordersByShippingStatus: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        return getOrdersByShippingStatus(input.shippingStatus, {
          page: input.page,
          limit: input.limit,
        });
      }),
    ...
    updateShipping: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const isNewTracking =
          data.trackingNumber && prevOrder && !prevOrder.trackingNumber;
    ...
    orderDashboardSummary: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
    ...
    updateExchange: protectedProcedure
      .input(
        z.object({
          id: z.number(),
          status: z
            .enum(["requested", "processing", "ready", "completed", "rejected"])
            .optional(),
          courierName: z.string().nullable().optional(),
          trackingNumber: z.string().nullable().optional(),
          adminMemo: z.string().nullable().optional(),
    ...
    updateReturn: protectedProcedure
      .input(
        z.object({
          id: z.number(),
          status: z
            .enum(["requested", "processing", "completed", "rejected", "hold"])
            .optional(),
          courierName: z.string().nullable().optional(),
          trackingNumber: z.string().nullable().optional(),
          adminMemo: z.string().nullable().optional(),
    ...
    createRefund: protectedProcedure
      .input(
        z.object({
          orderId: z.number(),
          refundMethod: z
            .enum(["card", "bank", "point", "deposit", "mixed"])
            .default("card"),
    ...
    productSalesStats: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      return getProductSalesStats();
    }),

    customerStats: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
  ...
  order: router({
    ...
    verify: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (Number(order.finalAmount) !== input.amount)
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        try {
          ...
          console.warn(
            "[Payment] commerce-payment edge function unavailable, falling back to local logic:",
            edgeError
          );
          ...
          try {
            ...
            if (!finalized.updated) {
              ...
              throw new TRPCError({
                code: "CONFLICT",
                message:
                  "주문 상태가 이미 변경되었습니다. 결제 내역을 확인해주세요.",
              });
            }
          } catch (error) {
            if (error instanceof Error && error.message.startsWith("[Stock]")) {
              ...
              try {
                ...
                if (failedOrder && typeof failedOrder.id === "number") {
                  await createCardCancellation({
                    ...
                    paymentKey: input.paymentKey,
                    cancelAmount: String(
                      failedOrder.finalAmount ?? failedOrder.totalAmount ?? 0
                    ),
                    cancelType: "full",
                    processedBy: "system",
                  ...
                  console.warn(
                    "[Payment] live order RPC path uses uuid order id, skipping legacy card_cancellations insert"
                  );
                }
              }
              ...
              await cancelOrderWithHistory({
                ...
                requestedBy: "admin",
                reason: cancelReason,
                adminNote: tossCancelFailed
                  ? `${error.message} (Toss 취소 실패 - 수동 환불 필요)`
                  : error.message,
                processedBy: "system",
                paymentKey: input.paymentKey,
              });
        ...
        if (updatedOrder) {
          ...
          void notifyOrderComplete({
            ...
            totalAmount: Number(
              updatedOrder.finalAmount ?? updatedOrder.totalAmount
            ),
            recipientName:
              updatedOrder.recipientName ?? ctx.user.name ?? "고객",
            orderDate,
          });
        }
    ...
    cancel: protectedProcedure
      .input(
        z.object({
          orderId: z.string(),
          cancelReason: z.string().default("관리자 취소"),
        })
      )
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
        ...
        try {
          ...
          console.warn(
            "[Payment] commerce-payment cancel edge function unavailable, falling back to local logic:",
            edgeError
          );
    ...
    cancelByUser: protectedProcedure
      .input(
        z.object({
          orderId: z.string(),
          cancelReason: z.string().default("고객 취소 요청"),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const order = await getOrderByOrderId(input.orderId);
        ...
        if (order.status !== "paid")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "결제 완료된 주문만 취소할 수 있습니다.",
          });
        // 24시간 이내 취소 가능 여부 확인
        if (!order.paidAt)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "결제 시간 정보가 없습니다.",
          });
        ...
        if (diffHours > 24)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "결제 후 24시간이 지나 취소가 불가능합니다. 고객센터(1:1 문의)로 연락해주세요.",
          });
        try {
          await invokeSupabaseEdgeFunction("commerce-payment/cancel", {
            user: { id: ctx.user.id, role: ctx.user.role },
          ...
          console.warn(
            "[Payment] commerce-payment buyer cancel edge function unavailable, falling back to local logic:",
            edgeError
          );
  ...
  adminExt: router({
    ...
    certifiedInstructorList: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        return getAdminCertifiedInstructors({
          page: input?.page ?? 1,
          limit: input?.limit ?? 100,
        });
      }),
    createCertifiedInstructor: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== "admin")
          throw new TRPCError({ code: "FORBIDDEN" });
        return createCertifiedInstructorSupabase({
          ...input,
          authorId: ctx.user.id as any,
        });
      }),
    ...
    nextProductCode: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (isImage) {
          try {
            ...
            const { url: mUrl } = await storagePut(
              medKey,
              medBuffer,
              "image/jpeg"
            );
    ...
    getDesignFolders: protectedProcedure.query(async ({ ctx }) => {
      if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const rows = result.items.map((item: any) => {
          ...
          for (const col of cols!) {
            switch (col.key) {
              case "orderId":
                row[col.label] = o.orderId;
                break;
              case "createdAt":
                row[col.label] = o.createdAt
                  ? new Date(o.createdAt).toLocaleString("ko-KR")
                  : "";
                break;
              case "recipientName":
                row[col.label] = o.recipientName ?? "";
                break;
              case "userEmail":
                row[col.label] = item.userEmail ?? "";
                break;
              case "orderName":
                row[col.label] = o.orderName ?? "";
                break;
              case "totalAmount":
                row[col.label] = Number(o.totalAmount ?? 0);
                break;
              case "paidAmount":
                row[col.label] = Number(o.totalAmount ?? 0);
                break;
              case "paymentMethod":
                row[col.label] = payLabel(o.paymentMethod ?? "");
                break;
              case "status":
                row[col.label] = statusLabel(o.status ?? "");
                break;
              case "shippingStatus":
                row[col.label] = shippingLabel(o.shippingStatus ?? "none");
                break;
              case "recipientPhone":
                row[col.label] = o.recipientPhone ?? "";
                break;
              case "recipientAddress":
                row[col.label] =
                  `${o.recipientAddress ?? ""} ${o.recipientAddressDetail ?? ""}`.trim();
                break;
              case "productName":
                row[col.label] = item.item?.productName ?? "";
                break;
              case "optionName":
                row[col.label] = item.item?.optionName ?? "";
                break;
              case "quantity":
                row[col.label] = item.item?.quantity ?? 0;
                break;
              case "price":
                row[col.label] = Number(item.item?.price ?? 0);
                break;
              default:
                row[col.label] = "";
            }
        ...
        return {
          base64,
          filename: `주문목록_${new Date().toISOString().slice(0, 10)}.xlsx`,
        };
      }),
  }),
});
