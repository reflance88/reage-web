...
import { findAuthUserByEmail, parseReturnPathState, sanitizeReturnPath } from "./authSession";
...
async function issueSession(
  ...
  const existing = await findAuthUserByEmail(lookupEmail);
...
export function registerSocialOAuthRoutes(app: Express): void {
  ...
  app.get("/api/oauth/kakao", (req: Request, res: Response) => {
    const returnPath = sanitizeReturnPath(req.query.returnPath as string | undefined);
  ...
  app.get("/api/oauth/kakao/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.redirect(302, parseReturnPathState(state));
    } catch (error) {
