...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!identifier || !password) {
      toast.error("아이디 또는 이메일과 비밀번호를 입력해주세요.");
      return;
    }
    setLoading(true);
    try {
  ...
  const returnTo =
    new URLSearchParams(window.location.search).get("returnTo") || "/mypage";
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          계정이 없으신가요?{" "}
          <Link href="/signup" style={{ color: "#6B0F1A", fontWeight: 600 }}>
            회원가입
          </Link>
        </p>
        ...
        >
          ...
          >
            신규 회원은 아이디 또는 이메일로 로그인할 수 있고, 기존 회원은
            이메일로 그대로 로그인할 수 있습니다.
          </p>
          <div>
            <label
              style={{
          ...
          <button
            ...
            onMouseOver={e => {
              if (!loading) e.currentTarget.style.background = "#8B1525";
            }}
            onMouseOut={e => {
              if (!loading) e.currentTarget.style.background = "#6B0F1A";
            }}
          >
            {loading ? "로그인 중..." : "로그인"}
          </button>
