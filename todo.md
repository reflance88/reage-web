...
## OAuth 로그인 복구 + 갤러리/매거진/인증강사 연동 복구 (2026-03-11)

- [x] OAuth upsertUser try/catch 제거 (DB 저장될 때만 로그인 진행)
- [x] DB 저장 실패 원인 파악 — Supabase에 users 테이블 미존재
- [x] 완전한 통합 SQL 파일 생성 (users 테이블 + camelCase 콜럼 추가)
- [x] 갤러리 관리자-홈페이지 연동 복구 (SQL 실행 완료, camelCase 콜럼 추가 확인)
- [x] 매거진 관리자-홈페이지 연동 복구 (SQL 실행 완료, camelCase 콜럼 추가 확인)
- [x] 인증강사 관리자-홈페이지 연동 복구 (SQL 실행 완료, camelCase 콜럼 추가 확인)

## 관리자 권한 관리 + 갤러리/매거진/인증강사 연동 확인 (2026-03-11)

- [x] 관리자 회원 관리 페이지에 role 변경 기능 확인 (이미 구현됨 확인)
- [x] 서버: updateUserRole API 확인 (이미 구현됨)
...
- [x] 팝업 "오늘 하루 닫지 않기" → "오늘 하루 보지 않기" 텍스트 수정
